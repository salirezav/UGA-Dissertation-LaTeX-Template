...
\makeabstract{%
    ...
    Biomedical image segmentation is essential for diagnostics, treatment planning, and biological research. Yet, the diversity of imaging modalities, complex structures, and the limited availability of annotated datasets pose significant challenges. Although convolutional neural networks (CNNs) often deliver state-of-the-art performance, they typically require extensive labeled data and domain-specific adjustments, restricting their generalizability. To address these limitations, self-supervised and unsupervised segmentation methods have emerged, exploiting unlabeled or weakly labeled data to alleviate the annotation burden. However, these methods can struggle with segmentation accuracy and robustness when confronted with complex domain-specific variability. Recent advances in foundation models, particularly the Segment Anything Model (SAM), suggest a promising path forward. Trained on large-scale, diverse data, SAM enables generalized segmentation via zero-shot learning, indicating potential applicability across a range of biomedical imaging domains. Nonetheless, fine-tuning and adaptation are necessary to ensure reliable performance in specialized biomedical contexts. This dissertation bridges these critical gaps. Chapter \ref{ch:intro} introduces a supervised 3D cell instance segmentation, tracking, and motility classification pipeline, centering on \textit{Toxoplasma gondii} and prioritizing both accuracy and user accessibility. Chapters \ref{ch:toxo} and \ref{ch:cilia} delve into self-supervised and minimally supervised approaches: the former employs motion-derived pseudo-labels to perform cilia segmentation without manual annotations, and the latter leverages contrastive learning under minimal supervision to enhance model generalization across diverse imaging modalities. Finally, Chapter \ref{ch:foundation} examines the adaptation of SAM for biomedical segmentation, detailing fine-tuning techniques that empower these foundation models to deliver reliable performance in specialized applications. Collectively, these contributions advance the field of biomedical imaging by mitigating annotation requirements, improving robustness, and widening applicability. By synthesizing supervised, self-supervised, and foundation model-based strategies, this dissertation offers a cohesive framework that addresses key hurdles in biomedical segmentation and paves the way for more efficient, scalable, and broadly accessible solutions.
